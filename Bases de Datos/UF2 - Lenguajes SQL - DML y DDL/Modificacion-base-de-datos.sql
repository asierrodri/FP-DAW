...
-- 1.	Inserta un nuevo registro en cada tabla.

INSERT INTO categoria VALUES (0, 'Teclado');
INSERT INTO fabricante VALUES (0, 'Google');
INSERT INTO producto VALUES(0, 'Teclado Google Lend 1.5', 100, 11, 11, 5);

-- 2.	Crea una base de datos nueva llamada backupproductos. Guarda en esta base de datos una copia de cada tabla de la base de datos productos.

CREATE DATABASE if not exists backupproductos;
USE productos;
CREATE TABLE backupproductos.nuevo_categoria AS SELECT * FROM categoria;
CREATE TABLE backupproductos.nuevo_fabricante AS SELECT * FROM fabricante;
CREATE TABLE backupproductos.nuevo_producto AS SELECT * FROM producto;

-- 3.	Incrementa el stock en 2 unidades de todos los productos de la categoría Monitor que tengan un stock menor de 4 unidades.

UPDATE producto
SET stock = stock + 2 
WHERE codigo_cat = (SELECT codigo_cat
					FROM categoria
                    WHERE nombre_cat = 'Monitor')
					AND stock < 4;

-- 4.	Queremos borrar al fabricante Lenovo y todos sus productos. Modifica la tabla para permitir borrar en cascada.

ALTER TABLE producto ADD CONSTRAINT fk_productofa_nuevo FOREIGN KEY(codigo_fa) REFERENCES fabricante(codigo_fa) ON DELETE CASCADE;
DELETE FROM producto
WHERE codigo_fa IN (SELECT codigo_fa
					FROM fabricante
                    WHERE nombre_fa = 'Lenovo');
DELETE FROM fabricante
WHERE nombre_fa = 'Lenovo';

-- 5.	Borra todos aquellos productos del fabricante AMD los cuales no tenemos stock.

DELETE FROM producto
WHERE codigo_fa = (SELECT codigo_fa
					FROM fabricante
                    WHERE nombre_fa = 'AMD')
                    AND stock = 0;